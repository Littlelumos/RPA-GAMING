...
if __name__ == "__main__":
    ...
    ctypes.windll.kernel32.SetConsoleTitleW("Latte Bot - Auto Click")
    if not windll.shell32.IsUserAnAdmin():
        # 不是管理员就提权
        windll.shell32.ShellExecuteW(
            None, "runas", sys.executable, __file__, None, 1)
    print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
    print("  Latte Bot - Auto Click version1.0")
    print("  拿铁侏儒造物，科技改变生活！")
    print("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
    print(" ")
    print(" ")
    print("  已通过认证服务，请您现在点击需要自动点击的窗口！")
    print("  点击后窗口将开始自动按下 \"←\"和\"0\"!")
    ...
    for i in range(0, 5):
        print(" ")
    print("  检测到游戏窗口，开始尝试绑定与自动操作！")
    while 1:
        key_down(handle, '0')
        key_down(handle, 'left')
        time.sleep(0.55)
        key_up(handle, 'left')
        key_up(handle, '0')
